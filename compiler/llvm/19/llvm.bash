...
opt=""
opt="$opt -DCMAKE_INSTALL_PREFIX=$(pack_get -prefix)"
...
#opt="$opt -DCMAKE_CXX_STANDARD=17"
...
# Create default configuration file for clang
for exe in clang clang-c1 clang++ flang
do
  pack_cmd "echo '--gcc-install-dir=$(pack_get -prefix gcc[$gnu_v])' > $(pack_get -prefix)/bin/$exe.cfg"
done
