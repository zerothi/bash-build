add_package http://sourceforge.net/projects/cp2k/files/cp2k-2.6.0.tar.bz2

#pack_set --host-reject ntch --host-reject zeroth

pack_set $(list -p '--module-requirement ' openmpi libxc fftw-3)

pack_set --install-query $(pack_get --prefix)/bin/cp2k

# Add the lua family
pack_set --module-opt "--lua-family $(pack_get --alias)"

if test -z "$FLAG_OMP" ; then
    doerr cp2k "Can not find the OpenMP flag (set FLAG_OMP in source)"
fi
...
DFLAGS  = -D__FFTW3 -D__HWLOC -D__LIBNUMA \n\
DFLAGS += -D__LIBXC2 -D__parallel -D__SCALAPACK\n\
CPPFLAGS = \n\
FCFLAGS = \$(DFLAGS) $FCFLAGS \$(FFTW_INC) \$(LIBXC_INC) \n\
