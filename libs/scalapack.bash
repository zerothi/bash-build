...
if [ $bl == "blas" ]; then
    pack_set --command "$tmp 's?BLASLIB[[:space:]]*=.*?BLASLIB = $tmp_lib $(list --LD-rp $bl) -lblas?g' $file"
elif [ $bl == "atlas" ]; then
    pack_set --command "$tmp 's?BLASLIB[[:space:]]*=.*?BLASLIB = $tmp_lib $(list --LD-rp $bl) -lf77blas -lcblas -latlas?g' $file"
elif [ $bl == "openblas" ]; then
    pack_set --command "$tmp 's?BLASLIB[[:space:]]*=.*?BLASLIB = $tmp_lib $(list --LD-rp $bl) -lopenblas?g' $file"
fi
pack_set --command "$tmp 's|^LAPACKLIB[[:space:]]*=.*|LAPACKLIB = $(list --LD-rp $bl) -llapack|g' $file"
